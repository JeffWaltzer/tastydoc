...
describe StringWrapper do
  it "handles the simple case" do
    ...
    StringWrapper.new('', false).wrap(repeated_string * 8).should == <<END
...
END
  ...
  it "handles a longer line" do
    repeated_string = '01234567890 '
    StringWrapper.new('', false).wrap(repeated_string * 16).should == <<END
#{(repeated_string * 6).strip}
#{(repeated_string * 6).strip}
...
END
  ...
  it "doesn't preserve many spaces which fall on a line boundary" do
    ...
    StringWrapper.new('', false).wrap(line).should == <<END
#{repeated_string * 7}
#{repeated_string}
END
  end
...
END
  ...
  it "respects indentation for lines without blanks that are too wide" do
    repeated_string = '01234567890'
    StringWrapper.new('  ', false).wrap(repeated_string * 8 + ' ' + repeated_string * 8).should == <<END
  #{repeated_string * 8}
  #{repeated_string * 8}
END
  end
  ...
  it "handles the case where a word longer than a line isn't the first word on a line" do
    repeated_string = '01234567890'
    StringWrapper.new('', false).wrap(repeated_string + ' ' + repeated_string * 8).should == <<END
#{repeated_string}
#{repeated_string * 8}
END
  end
